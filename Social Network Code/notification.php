...
if($user_ok != true || $log_username == ""){
	header("location: http://www.iedu-eg.com/00SN");
    exit();
}
$notification_list = "";
...
mysqli_query($link, "UPDATE users SET notescheck=now() WHERE username='$log_username' LIMIT 1");
?><?php
$friend_requests = "";
...
if($numrows < 1){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$thumbquery = mysqli_query($link, "SELECT avatar FROM users WHERE username='$user1' LIMIT 1");
		...
		if($user1avatar == NULL){
			$user1pic = '<img src="images/MaleProfile.png" alt="'.$user1.'" class="user_pic">';
		}
