...
// Upload Code
// Audio,Video , Assignments,...each one  has and ID PRIMARY AI NOT NULL and Name 
...
//print_r($_FILES);
$imageFileType = pathinfo($target_path,PATHINFO_EXTENSION); //Chech file extension
#Change this for Audio files...
if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path) AND $imageFileType == "png")
 {
 	$query="UPDATE `users` SET `video`='".mysqli_real_escape_string($link,$target_path)."' WHERE id='".$_SESSION['id']."' LIMIT 1";
 	//$query="INSERT INTO `users`(`video`)" ."values('$target_path') WHERE id='".$_SESSION['id']."' LIMIT 1";
	$result=mysqli_query($link, $query);
   if($result)
   {
    $message="The file has been uploaded Successfuly !";
...
/*if( && $imageFileType != "avi" && $imageFileType != "mov" && $imageFileType != "3gp" && $imageFileType != "mpeg")
{
